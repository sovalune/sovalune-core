...
use sovalune_storage_client::{MemoryRepository, MemoryFilter, CreateMemoryEntry, MemoryTier, MemoryEntry};
use sqlx::PgPool;
use uuid::Uuid;
...
impl VectorMemoryStore {
    ...
    pub async fn search(
        ...
        let results = self.repo.search_by_embedding(query_embedding, filter, top_k).await?;
    ...
    pub async fn consolidate(
        ...
        let entry = self.repo.consolidate(source_ids, &consolidated_content, embedding, metadata).await?;
        info!("Consolidated {} sources into {}", source_ids.len(), entry.id);
    ...
    pub async fn archive_low_decay(&self, threshold: f32) -> anyhow::Result<usize> {
        ...
        if affected > 0 {
            warn!("Archived {} entries with decay below {}", affected, threshold);
        }
        Ok(affected)
    }
